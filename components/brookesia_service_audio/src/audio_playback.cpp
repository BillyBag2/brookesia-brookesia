...
bool AudioPlayback::on_start()
{
    ...
    if (playback_iface != nullptr) {
        ...
        // Volume and mute are implemented by CodecPlayerIface and do not need
        // the optional AV processor.  Keep the service alive in control-only
        // mode so Settings can operate the board codec on platforms where the
        // full playback pipeline is unavailable.
        BROOKESIA_CHECK_FALSE_RETURN(
            ensure_player_iface(), false, "Failed to get audio player control interface"
        );
        BROOKESIA_LOGW("Audio playback pipeline is unavailable; running in control-only mode");
    }
